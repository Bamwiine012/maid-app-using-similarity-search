...
/* harmony import */ var _raw_loader_contact_page_html__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! !raw-loader!./contact.page.html */ 30478);
/* harmony import */ var _contact_page_scss__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./contact.page.scss */ 82962);
...
let ContactPage = class ContactPage {
    constructor(localService, restService, utilityService, eventsService, router) {
        ...
        this.hint = false;
        this.userType = 'guest';
        // check if user is user or cleaner
    ...
    send() {
        ...
        this.restService.postData('contact', this.contact).subscribe((data) => {
            this.utilityService.loadingClose();
            this.utilityService.openAlerts('Done!', 'Enquiry has been sent.', 'ok');
        }, (err) => {
            this.utilityService.loadingClose();
            this.utilityService.openAlerts('Error!', 'Error sending the enquiry.', 'ok');
        });
    }
    checkSelected() {
        console.log(this.contact);
        // remove the label as soon as selected
        const change1 = document.getElementsByClassName('select-label');
        change1[0].style.display = 'none';
        const change2 = document.getElementsByClassName('action-select');
